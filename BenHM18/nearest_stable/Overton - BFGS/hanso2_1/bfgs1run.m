function [x, f, d, H, iter, info, X, G, w, fevalrec, erec, trec] = bfgs1run(x0, pars, options)
% Version 2.02, 2013
...
for iter = 1:maxit
    if iter > 1
        fprintf('%2.0f:%2.2f...',iter,fevalrecline(end)); 
        if mod(iter,10) == 0
            fprintf('\n');   
        end 
    end
    ...
    if nargout > 9
        ...
        erec(iter) = fevalrecline(end); 
        trec(iter) = cputime-initialcputime; 
    end
    if prtlevel > 1
