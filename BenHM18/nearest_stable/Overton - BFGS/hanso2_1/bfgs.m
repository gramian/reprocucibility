function [x, f, d, H, iter, info, X, G, w, fevalrec, erec, trec] = bfgs(pars, options)
%BFGS The BFGS quasi-Newton minimization algorithm, Version 2.1, 2015
...
for run = 1:nstart
    ...
    if nargout > 9
        [x(:,run), f(run), d(:,run), HH, iter(run), info(run), X{run}, G{run}, w{run}, ...
           fevalrec{run}, erec{run}, trec{run}] = bfgs1run(x0(:,run), pars, options);
