...
if size(iterate_color,1) == 2
   initcolorlevel = iterate_color(1,:);
   finalcolorlevel = iterate_color(2,:);
end
for j = 3: n
    if (isempty(iterate_color)|| strcmp(savetype,'article')) ||...
            (size(iterate_color,1)<n-2 || size(iterate_color,2)~=3)
        %ensuring that all points are visible by setting a minimal level of
        %grey
        if size(iterate_color,1) == 2
            co = initcolorlevel+ j/(n-2)*(finalcolorlevel-initcolorlevel);
        else
...
if n > 1
    if complex
        ...
        if strcmp(savetype,'article')||strcmp(savetype,'poster')
            ...
            set(p3,'Markerfacecolor',originalpoint_color);
            set(p3,'MarkerEdgecolor',originalpoint_color);
        end
    else
        %eigenvalues of the initial problem
...
if complex && (strcmp(savetype,'article')||strcmp(savetype,'poster'))
