function [varargout] = PlotStableSet(A,B,B0,isdiscrete,varargin)
...
nptx = 200; %number of samples used for the axis B0-B
npty = 300; %number of samples used for the axis B-A

%coefficient range of the search (B is at center, B0 on the right, A at the
%bottom)
leftrange   = 1; % leftlimit = leftrange *  -norm(B0-B);
rightrange  = 1; %rightlimit =  rightrange * norm(B0-B);
toprange    = 1; % toplimit = toprange* -norm(A-B);
bottomrange = 1; %bottomlimite = bottomrange * norm(A-B);
...
X = B0-B;
Y = B-A;
%Base Choice user defined
% X = [0 1/sqrt(2) 1/sqrt(2) 0]';
% Y = [0 -1/sqrt(6) 1/sqrt(6) sqrt(2/3)]';
...
leftlimit   = -norm(X,normtype)*leftrange;
rightlimit  =  norm(X,normtype)*rightrange;
bottomlimit = -norm(Y,normtype)*bottomrange;
toplimit    =  norm(Y,normtype)*toprange;
...
if isempty(bestSol)
    fprintf('\n-> No stable values found with the current parametrization\n')
    return
else
...
%% Computing the orthonormalized coordinates
    ...
    AB = A-B;
    coordB  = [0 0];
    coordB0 = [real(trace(D1'*BB0)) real(trace(D2orth'*BB0))];
    coordA  = [real(trace(D1'*AB)) real(trace(D2orth'*AB))] ;
...
%% second fig : Stable set (orthonormalized base)
    ...
    plot(o,p,'.','MarkerSize',6,'MarkerEdgecolor',[1 0   0])
