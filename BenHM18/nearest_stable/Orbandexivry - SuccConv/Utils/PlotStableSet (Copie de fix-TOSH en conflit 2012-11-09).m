...
nptx = 601; %number of samples used for the axis B0-B
npty = 301; %number of samples used for the axis B-A

%coefficient range of the search (B is at center, B0 on the right, A at the
%bottom)
leftrange   = 0.1; % leftlimit = leftrange *  -(B0-B)/norm(B0-B);
rightrange  = 0.1; %rightlimit =  rightrange * (B0-B)/norm(B0-B);
toprange    = 0.01; % toplimit = toprange* -(A-B)/norm(A-B);
bottomrange = 0; %bottomlimite = bottomrange * (A-B)/norm(A-B);
...
%% second fig : Stable set (orthonormalized base)
    ...
    warning('Borders are plotted in blue instead of brown')
    plot(k,l,'.','MarkerSize',4)
